package gecko.components.collision.aabb;
...
class HitBoxComponent extends Component {
    ...
    public function init(?tagsToCollide:Array<String>, ?box:Rect){
        ...
        onAddedToEntity += _initBounds;
        onRemovedFromEntity -= _disableBounds;
    }
    ...
    private function _initBounds(e:Entity) {
        updateBounds();
        e.transform.onTransformChange += updateBounds;
    }

    private function _disableBounds(e:Entity) {
        e.transform.onTransformChange -= updateBounds;
    }
